...
describe('metodoFalsaPosicao', () => {
  ...
  it('deve encontrar a raiz da função f(x) = x^2 - 4 no intervalo [0, 3] com tolerância 0.001', () => {
    ...
    expect(resultado.resultado.raiz).toBeCloseTo(2, 3); // Verifique se a raiz encontrada é próxima de 2
    expect(resultado.resultado.iteracoes).toBeLessThanOrEqual(maxIteracao); // Verifique o número de iterações
    expect(resultado.resultado.erro).toBeLessThan(tolerancia); // Verifique se o erro final está dentro da tolerância
  });
  ...
  it('deve lançar erro se o número máximo de iterações for atingido sem convergência', () => {
    ...
    expect(resultado.resultado.motivoParada).toBe('Número máximo de iterações atingido sem convergência.');
  });
  ...
  it('deve encontrar a raiz da função f(x) = x^3 - x - 2 no intervalo [1, 2] com tolerância 0.001', () => {
    ...
    expect(resultado.resultado.raiz).toBeCloseTo(1.521, 3);
    expect(resultado.resultado.iteracoes).toBeLessThan(maxIteracao);
    expect(resultado.resultado.erro).toBeLessThan(tolerancia);
  });
  ...
  it('deve encontrar a raiz no intervalo [1, 2] para a função f(x) = x^2 - 1', () => {
    ...
    const resultado = metodoFalsaPosicao(funcao, intervalo, tolerancia, maxIteracao);
    expect(resultado.resultado.raiz).toBeCloseTo(1, 3); // Verifica se a raiz encontrada é 1
  });
  ...
  it('deve retornar b quando f(b) = 0', () => {
    ...
    expect(resultado.resultado.raiz).toBe(2); // Verifica se a raiz retornada é 2
    expect(resultado.resultado.iteracoes).toBe(0); // Verifica que não houve iterações
    expect(resultado.resultado.erro).toBe(0); // Verifica que o erro é zero
    expect(resultado.resultado.convergiu).toBe(true); // Verifica que convergiu
  });
